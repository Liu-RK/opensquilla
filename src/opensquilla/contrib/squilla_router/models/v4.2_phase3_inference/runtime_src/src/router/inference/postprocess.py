...
def _apply_optional_sticky_tier(
    ...
    margin: float,
    margin_upgraded: bool,
    config: dict,
) -> tuple[str, bool]:
    sticky_cfg = config.get("v4", {}).get("sticky_tier", {})
    if not sticky_cfg.get("enabled", False) or margin_upgraded:
        return route_class, False
    ...
    max_margin = sticky_cfg.get("max_margin")
...
def apply_postprocess(
    ...
    route, sticky_applied = _apply_optional_sticky_tier(
        route, fused_probs, request, margin, margin_upgraded, config
    )
