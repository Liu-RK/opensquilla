...
def _apply_sticky_tier(pred_class: str, probs: np.ndarray,
    ...
    When the predicted class is adjacent to the previous turn's class and
    the margin between them is small, stick with the previous class to
    avoid unnecessary KV-cache invalidation from tier switching.
    """
    ...
    if abs(pred_idx - prev_idx) != 1:
        return pred_class
    margin = probs[pred_idx] - probs[prev_idx]
    if margin < cfg["thresholds"].get("sticky_margin", 0.15):
