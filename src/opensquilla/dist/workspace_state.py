...
# ``opensquilla.tools.builtin``. ``shell_policy`` is excluded as a helper
# consumed by ``shell``.
...
def _read_package_metadata() -> tuple[str, str]:
    """Return (version, python_requires) from installed opensquilla metadata."""

    try:
        meta = _pkg_metadata("opensquilla")
